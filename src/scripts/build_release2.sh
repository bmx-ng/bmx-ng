...
# Builds the bcc2/bmk2 release line from an installed or downloaded legacy
# BlitzMax NG release. The legacy build_release.sh remains unchanged.
#
# WINDOWS
#
...
usage() {
	...
	echo "Builds a bcc2/bmk2 distribution using <version> as the legacy seed release."
...
MOD_LIST=("brl" "pub" "maxgui" "audio" "crypto" "image" "net" "random" "sdl" "steam" "text" "math" "archive" "collections" "embedded" "pico" "esp32")
...
download() {
	...
	# base
	download_repo_zip "bmx-ng" "bmx-ng/bmx-ng" "$BMXNG_REF" "zips/bmx-ng.zip"
	...
	do
		...
		download_repo_zip "${mod}.mod" "bmx-ng/${mod}.mod" "$mod_ref" "zips/${mod}.mod.zip"
	done
...
prepare() {
	...
	# bcc2 retains the installed src/bcc layout expected by bmk and bootstrap.
	echo "Extracting bcc2"
	unzip_and_rename "zips/bcc2.zip" "release/BlitzMax/src" "bcc2" "bcc"
	unzip_and_rename "zips/bcc2.zip" "temp/BlitzMax/src" "bcc2" "bcc"

	# bmk2 likewise retains src/bmk in an installed SDK.
	echo "Extracting bmk2"
	unzip_and_rename "zips/bmk2.zip" "release/BlitzMax/src" "bmk2" "bmk"
	unzip_and_rename "zips/bmk2.zip" "temp/BlitzMax/src" "bmk2" "bmk"

	# Compiler, language and LSP modules used by bcc2 and bls.
	echo "Extracting blitzmax.mod"
	unzip_and_rename "zips/blitzmax.mod.zip" "release/BlitzMax/mod" "blitzmax.mod" "blitzmax.mod"
	unzip_and_rename "zips/blitzmax.mod.zip" "temp/BlitzMax/mod" "blitzmax.mod" "blitzmax.mod"
	...
	case "$PLATFORM" in
		macos)
			echo "Copying bmk2 bootstrap config"
			cp release/BlitzMax/src/bootstrap/bootstrap2.cfg release/BlitzMax/bin
			cp temp/BlitzMax/src/bootstrap/bootstrap2.cfg temp/BlitzMax/bin
			
			echo "Configuring bootstrap for $PLATFORM/$ARCH"
			echo -e "t\tmacos\t"$ARCH"\n$(cat temp/BlitzMax/bin/bootstrap2.cfg)" > temp/BlitzMax/bin/bootstrap2.cfg
			
			echo "Copying scripts"
			cp release/BlitzMax/src/macos/build_dist2.sh release/BlitzMax/build_dist.sh
			cp release/BlitzMax/src/macos/run_me_first2.command release/BlitzMax/run_me_first.command
...
build_apps() {
	...
	# Put the complete legacy tool pair in the staged SDK first. Both initial
	# bcc2 and bmk2 are deliberately compiled by the legacy pair; only after
	# both builds succeed do we replace either executable.
	echo ""
	echo "Copying legacy compiler and build manager"
	cp BlitzMax/bin/bcc temp/BlitzMax/bin && \
	...
	echo "Building initial bcc2 with the legacy toolchain"
	if ! temp/BlitzMax/bin/bmk makeapp -r -single temp/BlitzMax/src/bcc/compiler/bcc.bmx; then
		echo "Failed to build initial bcc2"
		exit 1
	fi

	echo "Building initial bmk2 with the legacy toolchain"
	...
	echo ""
	echo "Building initial bmk2"
	if temp/BlitzMax/bin/bmk makeapp -r -single $TARG_ARCH temp/BlitzMax/src/bmk/bmk.bmx; then
		retries=0
		while [ $retries -lt 30 ]
		do
			# Install the first-pass bcc2 immediately before bmk2 so no mixed
			# legacy/new pair is used for either source build.
			cp temp/BlitzMax/src/bcc/compiler/bcc temp/BlitzMax/bin/bcc && \
			rm temp/BlitzMax/bin/bmk && \
			cp temp/BlitzMax/src/bmk/bmk temp/BlitzMax/bin/bmk 2>/dev/null
			if [ $? -eq 0 ]; then
				break
			else
		...
		echo ""
		echo "Failed to build initial bmk2"
		exit -1
	fi
	...
	case "$PLATFORM" in
		...
		*)
			...
			if ! temp/BlitzMax/bin/bmk makeapp -a -r $G_OPTION $C_OPTION temp/BlitzMax/src/bcc/compiler/bcc.bmx; then
				echo ""
				echo "Failed to build latest bcc2"
				exit -1
			fi
			cp temp/BlitzMax/src/bcc/compiler/bcc$C_EXT release/BlitzMax/bin

			if [ -z "$CROSS_COMPILE" ];then
				echo "Copying latest bcc to bin"
				# build using the latest bcc
				cp temp/BlitzMax/src/bcc/compiler/bcc$C_EXT temp/BlitzMax/bin
			fi
			...
			# build bootstrap
			if [ -n "$BUILD_BOOTSTRAP" ];then
				echo "Configuring bmk2 bootstrap"
				cp temp/BlitzMax/src/bootstrap/bootstrap2.cfg temp/BlitzMax/bin
				...
				echo -e "\nt\t${PLAT}\t${ARCH}\n" >> temp/BlitzMax/bin/bootstrap2.cfg
			...
			# build the BlitzMax language server with the final bcc2/bmk2 pair
			echo "Building bls"
			if ! temp/BlitzMax/bin/bmk makeapp -a -r $G_OPTION $C_OPTION temp/BlitzMax/src/bcc/lsp/bls.bmx; then
				echo ""
				echo "Failed to build bls"
				exit 1
			fi
			cp temp/BlitzMax/src/bcc/lsp/bls$C_EXT release/BlitzMax/bin
...
get_version() {
	# Extracting the version from bcc2's version.bmx file
	bcc_version=$(grep 'Const BCC_VERSION:String' temp/BlitzMax/src/bcc/compiler/version.bmx | sed -n 's/.*Const BCC_VERSION:String = "\(.*\)".*/\1/p')
