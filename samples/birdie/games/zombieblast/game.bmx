...
Local a
For a=0 Until 100
  cloud.Create( Float(Rnd(-WorldSize,WorldSize)), Float(Rnd(-WorldSize,WorldSize)) )
Next
For a=0 Until 250
  Baddie.Create(Float(Rnd(-1000,1000)), Float(Rnd(-1000,1000)), 0)
Next
...
Type SmokeEmitter Extends Detail
  ...
  Method Update( wx#, wy# )
    Local sp:smoke_prt
    SetColor Int(255*rd),Int(255*gn),Int(255*bl)
    ...
    Next
    If cur_cnt<max_cnt
      smlist.addfirst( smoke_prt.Create( 0.2, 0.3, 0, 0, Float(Rnd( -0.1, 0.1 )), Float(Rnd(0.1,1)), 40 ) )
      cur_cnt:+1
    EndIf
    life:-1
    If life<0
